...
joint.ui.TextEditor = joint.mvc.View.extend({
    ...
    init: function() {
        ...
        if (this.options.annotations) {
            this.setLabelings(this.options.annotations);
        }
    },
    ...
    annotateURLBeforeCaret: function(selectionStart) {
        ...
        if (urlBoundary) {
            this.annotateURL(this.getLabelings() || [], urlBoundary[0], urlBoundary[1]);
            return true;
        }
    ...
    onInput: function(evt) {
        ...
        var annotations = this.getLabelings();
        ...
        // annotate it using the `urlLabeling` option.
        if (this.options.annotateUrls && opType === 'insert') {
            ...
            if (/\s/.test(insertedText)) {
                ...
                if (annotated) {
                    ...
                    annotations = this.shiftLabelings(annotations, selectionAfterInput.end, diffLength);
                }
            }
        }
        ...
        if (annotations) {
            ...
            // should call `setCurrentLabeling()`. Then when the user starts typing ('insert' operation),
            // we want to create a new annotation with the desired attributes.
            if (this._currentLabelingAttributes) {
                ...
                if (opType === 'insert') {
                    ...
                    var insertLabeling = {
                        ...
                        attrs: this._currentLabelingAttributes
                    };
                    annotations.push(insertLabeling);
                    ...
                    // attributes of the previous one (which has our `insertLabeling` applied).
                    this._currentLabelingAttributes = undefined;

                    if (this.options.debug) {
                        console.log('onInput()', 'insert annotation', insertLabeling, 'final annotations', annotations);
                    }
                }
            }
        }
    ...
    findLabelingsUnderCursor: function(annotations, selectionStart) {

        return V.findLabelingsAtIndex(annotations, selectionStart);
    },
    ...
    findLabelingsInSelection: function(annotations, selectionStart, selectionEnd) {

        return V.findLabelingsBetweenIndexes(annotations, selectionStart, selectionEnd);
    },
    ...
    annotate: function(annotations, selectionBeforeInput, selectionAfterInput, diffLength) {
        ...
        _.each(annotations, function(annotation) {
            ...
            var inLabeling, removedInLabeling;
            ...
            switch (opType) {
                ...
                case 'delete':
                    if (annotation.start <= selectionBeforeInput.start && selectionBeforeInput.start <= annotation.end) {
                        ...
                        inLabeling = annotation.end - annotation.start;
                        removedInLabeling = selectionBeforeInput.end - annotation.start;
                        annotation.start = selectionBeforeInput.start;
                        annotation.end = annotation.start + inLabeling - removedInLabeling;
    ...
    shiftLabelings: function(annotations, selectionStart, offset) {

        return V.shiftLabelings(annotations, selectionStart, offset);
    },
    ...
    // At this point, we can just call `setCurrentLabeling({ 'font-weight': 'bold' })` and let the
    ...
    setCurrentLabeling: function(attrs) {

        this._currentLabelingAttributes = attrs;
    },
    ...
    setLabelings: function(annotations) {

        this._annotations = annotations;
    },

    // @public
    getLabelings: function() {
    ...
    getCombinedLabelingAttrsAtIndex: function(selectionStart, annotations) {
    ...
    // `defaultLabeling` (default annotation does not need `start` and `end` properties).
    ...
    getSelectionAttrs: function(range, annotations) {
        ...
        if (start === end && start === 0) {
            ...
            return this.getCombinedLabelingAttrsAtIndex(start, annotations);

        } else if (start === end) {
            // Nothing is selected. We take attributes of the character before the current caret position.
            return this.getCombinedLabelingAttrsAtIndex(start - 1, annotations);

        } else {

            var commonAttrs;
            for (var i = start; i < end; i++) {
                var attrs = this.getCombinedLabelingAttrsAtIndex(i, annotations);
    ...
    annotateURL: function(annotations, selectionStart, selectionEnd) {
        ...
        var urlLabeling = _.extend({ url: url }, this.options.urlLabeling);

        urlLabeling.start = selectionStart;
        urlLabeling.end = selectionEnd;
        // Do not add the annotation if there was the exact same
        // one at the end already.
        if (!_.isEqual(urlLabeling, _.last(annotations))) {
            annotations.push(urlLabeling);
        }
    ...
    // `opt.annotations` ... Labelings that will be set on the `ui.TextEditor` instance.
    ...
    edit: function(el, opt) {
        ...
        if (opt.cellView) {
            ...
            if (opt.annotationsProperty && !this.ed.getLabelings()) {
                ...
                if (annotations) {
                    ...
                    this.ed.setLabelings(this.deepCloneLabelings(annotations));
                }
            }
            ...
            this.ed.on('text:change', function(newText, oldText, annotations) {
                ...
                if (opt.cellView) {
                    ...
                    if (opt.annotationsProperty) {
                        ...
                        // all the backbone `changed` mechanism works. This is because
                        // the text editor modifies the `annotations` array in-place.
                        opt.cellView.model.prop(opt.annotationsProperty, this.deepCloneLabelings(annotations), { rewrite: true, textEditor: this.ed.cid });
                    }
    ...
    close: function() {
        ...
        if (this.ed) {
            ...
            if (this.ed.options.annotateUrls) {
                ...
                var annotationsUnderCursor = this.findLabelingsUnderCursor();
                var containsURLLabeling = _.find(annotationsUnderCursor, function(annotation) {
                    if (annotation.url) return annotation;
                    return false;
                });
                if (!containsURLLabeling) {
                    var annotated = this.ed.annotateURLBeforeCaret(selectionStart);
                    if (annotated) {
                        this.applyLabelings(this.getLabelings());
                    }
                }
            }
    ...
    applyLabelings: function(annotations) {
        ...
        if (this.ed && opt.update) {
            ...
            if (opt.cellView && opt.annotationsProperty) {
                ...
                // all the backbone `changed` mechanism works. This is because
                // the text editor modifies the `annotations` array in-place.
                opt.cellView.model.prop(opt.annotationsProperty, this.deepCloneLabelings(annotations), { rewrite: true });
                this.ed.setLabelings(annotations);
    ...
    // @private
    deepCloneLabelings: function(annotations) {
    ...
    setCurrentLabeling: function(attributes) {

        return this.proxy('setCurrentLabeling', arguments);
    },

    getLabelings: function() {

        return this.proxy('getLabelings', arguments);
    },
    ...
    findLabelingsUnderCursor: function() {

        return this.proxy('findLabelingsUnderCursor', [this.ed.getLabelings(), this.ed.getSelectionStart()]);
    },

    findLabelingsInSelection: function() {

        if (!this.ed) return;

        // Get the *normalized* selection range.
        var range = this.ed.getSelectionRange();
        return this.proxy('findLabelingsInSelection', [this.ed.getLabelings(), range.start, range.end]);
    },
