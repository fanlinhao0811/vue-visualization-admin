...
V = Vectorizer = (function() {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line) {
                ...
                if (opt.annotations) {
                    ...
                    var lineLabelings = V.annotateString(lines[i], V.isArray(opt.annotations) ? opt.annotations : [opt.annotations], { offset: -offset, includeLabelingIndices: opt.includeLabelingIndices });
                    for (var j = 0; j < lineLabelings.length; j++) {

                        var annotation = lineLabelings[j];
                        if (V.isObject(annotation)) {
                            ...
                            if (opt.includeLabelingIndices) {
                                // If `opt.includeLabelingIndices` is `true`,
    ...
    V.findLabelingsAtIndex = function(annotations, index) {
    ...
    V.findLabelingsBetweenIndexes = function(annotations, start, end) {
    ...
    V.shiftLabelings = function(annotations, index, offset) {
